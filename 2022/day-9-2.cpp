...
// clang++ -g -std=c++20 -o day-9-2 day-9-2.cpp 
// ./day-9-2 < day-9-example.txt
// ./day-9-2 < day-9-example-2.txt
// ./day-9-2 < day-9-prod.txt
...
int main() {
    ...
    for (int i = 0; i < knotCount; i++) knots.push_back(Point(startRow, startColumn));
        ...
        for (int i = 0; i < count; i++) {
            for (int k = 0; k < knotCount; k++) {
                if (k == 0) {
                    ...
                    knots[k].move(knots[k-1]);
                }
            }
            board[knots[9].row][knots[9].column] = true;
        }
    }
