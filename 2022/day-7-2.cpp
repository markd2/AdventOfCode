...
// clang++ -g -std=c++20 -o day-7-2 day-7-2.cpp 
// ./day-7-2 < day-7-example.txt
// ./day-7-2 < day-7-prod.txt
...
const int kCapacity = 70'000'000;
const int kTarget = 30'000'000;
int currentUnused = 0;
...
void walk(const inode *root, int indent = 0) {
    for (std::map<std::string, inode*>::const_iterator it = root->dirent.begin();
        ...
        if (value->isDirectory) {
            auto sum = du(value);
            std::cout << "BLORF " << currentUnused << "+" << sum << " = " << currentUnused+sum << " vs " << kTarget << std::endl;
            if (currentUnused + sum >= kTarget) {
                matchingDu.push_back(sum);
            }
...
int main() {
    ...
    currentUnused = kCapacity - du(root);

    walk(root, 1);
    std::cout << "blah" << matchingDu.size() << std::endl;

    // too lazy to figure out std::reduce for this
    int sum = 0;
    for (auto size: matchingDu) {
        std::cout << "SPLUNGE " << size << std::endl;
    }
    std::sort(matchingDu.begin(), matchingDu.end(), std::less<int>());
    std::cout << matchingDu[0] << std::endl;
}
